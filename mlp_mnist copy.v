...
module PE # (parameter tam = 16, parameter data_size = 10)
    (
...
module matrix_PE # (parameter tam = 16, parameter data_size = 2, parameter pe_qnt = 3)
    (
        input [tam-1:0] bus_in,
        input [tam-1:0] bias,
        input [pe_qnt-1:0][data_size-1:0][tam-1:0] initial_weight,
        input clk,
        input rst,
        output [pe_qnt-1:0][tam-1:0] acc_out
    );

    genvar i;
    generate
        for (i = 0; i < pe_qnt; i = i + 1) begin: PE_Matrix
            PE #(.tam(tam), .data_size(data_size)) PE_units (.bus_in(bus_in), .bias(bias), .initial_weight(initial_weight[i]), .clk(clk), .rst(rst), .acc_out(acc_out[i]));
        end
    endgenerate
